...
class knn_framework():
    ...
    def data_split(self) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        for step in range(1, feature_size - 1):
            ...
            for combination in list(combinations(range(0, feature_size), r=step)):
                ...
                if combination:
                    ...
                    result = (step, combination, model.score(X_test_temp, y_test), [
    ...
    def n_optimizer(self):
        ...
        if self.optimal_features is None:
            ...
            print(
                f'Optimizing n for optimal features: {self.optimal_features[3]}\n')
            n_max = int(len(X_train) / 2)
            for n in range(1, n_max):
                model = KNeighborsClassifier(
                    n_neighbors=n).fit(X_train, y_train)
    ...
    def final_score(self):
        if self.optimal_features is None:
            ...
            y = self.y
            model = KNeighborsClassifier(
                n_neighbors=self.optimal_n[0]).fit(X, y)
